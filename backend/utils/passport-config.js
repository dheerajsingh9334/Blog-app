...
passport.use(
  new JWTStrategy(options, async (userDecoded, done) => {
    try {
      // Ensure userDecoded.id is a string
      const userId = userDecoded.id?.toString();
      if (!userId) {
      ...
      try {
        ...
        if (!user) {
          ...
          user = await User.create({
            ...
            authMethod: "google",
            email,
            plan: freePlan?._id, // Assign free plan if available
            hasSelectedPlan: !!freePlan, // Set to true if free plan is assigned
          });
        }
        done(null, user);
      } catch (error) {
