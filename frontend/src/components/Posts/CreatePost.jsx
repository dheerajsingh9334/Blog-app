  ...
  return (
    <PlanAccessGuard feature="create_post">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12 bg-white dark:bg-gray-900 transition-colors duration-300 min-h-screen">
        ...
        {isSuccess && (
          <AlertMessage type="success" message="Story published successfully!" />
        )}
              ...
              <div className="flex items-center">
                ...
                <div className="ml-3">
                  ...
                  <div className="mt-2 text-sm text-red-700">
                    <p>Your account has been banned. You cannot create new posts.</p>
                    {userProfile?.user?.banReason && (
                      <p className="mt-1"><strong>Reason:</strong> {userProfile.user.banReason}</p>
                    )}
                    {userProfile?.user?.bannedBy && (
                      <p className="mt-1">
                        <strong>Contact Admin:</strong> {userProfile.user.bannedBy.email}
                      </p>
                    )}
                  </div>
                </div>
              ...
              {formik.values.status === "scheduled" && hasScheduledPosts(userPlan) && (
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) => formik.setFieldValue("scheduledFor", e.target.value)}
                    disabled={isBanned}
                  ...
                  {formik.touched.scheduledFor && formik.errors.scheduledFor && (
                    <p className="text-sm text-red-600 dark:text-red-400 mt-1">{formik.errors.scheduledFor}</p>
                  )}
                </div>
              )}
            </div>
