...
import { FaCheck, FaCrown, FaChartLine, FaGift, FaInfinity, FaStar } from "react-icons/fa";
...
const CheckoutForm = () => {
  ...
  const effectivePlanId = planId || planCandidate?._id || planCandidate?.tier || planCandidate?.planName;
  ...
  console.log("Available plans:", Array.isArray(plansData?.plans) ? plansData.plans : []);
  console.log("Stripe Loaded:", !!stripe);
  console.log("Elements Loaded:", !!elements);

  // Find the current plan with fallback if plans are not loaded
  let currentPlan = planCandidate
    || plansData?.plans?.find(plan => 
      ...
      <div className="bg-gray-900 h-screen -mt-4 flex justify-center items-center">
        <div className="w-96 mx-auto my-4 p-6 bg-white rounded-lg shadow-md text-center">
          ...
          <p className="text-gray-600 mb-4">
            The selected plan &quot;{effectivePlanId}&quot; is not valid. Please select a plan from our pricing page.
          </p>
          <button
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const planIdToUse = currentPlan?._id || currentPlan?.tier || effectivePlanId;
      ...
      if (!paymentResult?.clientSecret) {
        ...
        setErrorMessage(`Failed to create payment intent for plan &quot;${planIdToUse}&quot;. Please try again or contact support.`);
        setIsProcessing(false);
        return;
      }
    ...
    <div className="bg-gray-900 min-h-screen -mt-4 flex justify-center items-center p-4">
      <div className="w-full max-w-4xl mx-auto">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
          ...
          <div className={`p-6 rounded-lg shadow-md ${planColors.bg} ${planColors.border} border`}>
            <div className="text-center mb-6">
              ...
              <p className="text-gray-600 dark:text-gray-400">
                {currentPlan?.postLimit ? `${currentPlan.postLimit} posts` : 'Unlimited posts'}
              </p>
            </div>
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg p-4 mb-6">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex items-center">
                  ...
                  <span className="text-gray-700 dark:text-gray-300">
                    {currentPlan?.tier === 'free' ? 'Basic' : currentPlan?.tier === 'premium' ? 'Advanced' : 'Premium'} features
                  </span>
                </div>
                <div className="flex items-center">
                  <FaCheck className="text-green-500 mr-2" />
                  <span className="text-gray-700 dark:text-gray-300">
                    {currentPlan?.tier === 'free' ? 'Standard' : 'Priority'} support
                  </span>
                </div>
              </div>
            </div>
          ...
          <div className="bg-white dark:bg-gray-800 rounded-lg shadow-md p-6">
            <div className="mb-6 text-center">
              ...
              <p className="text-gray-600 dark:text-gray-400 text-sm">
                {currentPlan ? `Upgrading to ${currentPlan.planName}` : 'Enter your payment details to continue'}
              </p>
              {currentPlan?.price > 0 && (
            ...
            {/* Display loading */}
            {isProcessing && (
              <AlertMessage type="loading" message="Processing payment, please wait..." />
            )}
            ...
            {paymentMutation?.isError && (
              <AlertMessage
                type="error"
                message={paymentMutation?.error?.response?.data?.message || "Payment failed"}
              />
            )}
            ...
            >
              {isProcessing ? "Processing..." : `Pay $${currentPlan?.price || 0}`}
            </button>
